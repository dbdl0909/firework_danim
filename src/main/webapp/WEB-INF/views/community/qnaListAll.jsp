...
<div class="container">
	<div class="qnaHeader">
		...
		<form id="qnaCategory" action="/community/qnaListAll" method="GET">
	...
	<c:if test="${param.searchOption == null and param.searchInput == null }">
		<div id="paginationWrap">
			...
			<li>
				<c:if test="${page>1}">
					<a href="/community/qnaListAll?communityCategoryNo=${param.communityCategoryNo}&page=${page-1}">&laquo;</a>
				</c:if>
			</li>
			<c:forEach begin="${startPage}" end="${endPage}" var="i">
			<li>
				<c:choose>
					<c:when test="${page == i}">
						<a href="/community/qnaListAll?communityCategoryNo=${param.communityCategoryNo}&page=${i}" style="background:#434343; color:#fff;">${i}</a>
					</c:when>
					<c:otherwise>
						<a href="/community/qnaListAll?communityCategoryNo=${param.communityCategoryNo}&page=${i}">${i}</a>
					</c:otherwise>
				</c:choose>
			</li>
			</c:forEach>
			<li>
				<c:if test="${page<lastPage}">
					<a href="/community/qnaListAll?communityCategoryNo=${param.communityCategoryNo}&page=${page+1}">&raquo;</a>
				</c:if>
			</li>
			</ul>
		</div>
...
var onList = function(){
	location.href = '/community/qnaListAll?communityCategoryNo='+'${param.communityCategoryNo}';
};
